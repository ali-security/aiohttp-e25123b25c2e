...
@pytest.mark.skipif(NO_EXTENSIONS, reason="Only tests C parser.")
@pytest.mark.parametrize(("body", "snippet", "reason"), _BAD_CHUNKED_RESPONSES_AT_END)
async def test_c_parser_error_snippet_does_not_over_read_buffer(
    loop: asyncio.AbstractEventLoop,
    protocol: Any,
    body: bytes,
    snippet: bytes,
    ...
    # Feed a buffer that is *not* NUL terminated at its end: the memory right after
    # the fed region holds a sentinel. Building the snippet from the raw llhttp
    # error-position pointer makes Cython materialize it with strlen, which runs past
    # the end of the fed buffer (already released by then) and leaks the sentinel
    # into the error message. Slicing the bounded ``data`` object cannot.
    response = HttpResponseParserC(
        protocol, loop, 2**16, max_line_size=8190, max_field_size=8190
    )
    text = _CHUNKED_RESP_HDR + body
    backing = bytearray(text + b"OUTOFBOUNDS" * 8)
    view = (ctypes.c_char * len(text)).from_buffer(backing)
    with pytest.raises(http_exceptions.BadHttpMessage) as exc_info:
        response.feed_data(view)
    message = str(exc_info.value)
    assert "OUTOFBOUNDS" not in message
    assert reason in message
    assert repr(snippet) in message
